...
import com.ezone.devops.scheduler.job.NpmStep;
...
@Slf4j
@Service
public class NpmOperator extends AbstractPluginOperator implements PluginOperator {
    ...
    public boolean execute(RepoVo repo, Pipeline pipeline, PipelineRecord pipelineRecord, JobRecord jobRecord) {
        ...
        Npm npm = npmDataOperator.getRealJob(jobRecord.getPluginId());
        String image = getBuildImageUrl(npm.getBuildImageId());

        FileUploadStep fileUploadStep = new FileUploadStep();
        fileUploadStep.setUploadArtifact(npm.isUploadArtifact());
        if (npm.isUploadArtifact()) {
            String version = pipelineRecord.getSnapshotVersion();
            if (npm.getVersionType() == VersionType.CUSTOM_VERSION) {
                version = FreemarkerUtil.parseExpression(npm.getCustomVersion(), envs);
                if (StringUtils.isBlank(version)) {
                    super.noticeJobFailed(pipelineRecord, jobRecord, "解析version出错");
                    return false;
                }
            } else if (npm.getVersionType() == VersionType.RELEASE) {
            ...
            fileUploadStep.setPkgRepo(npm.getPkgRepo());
            fileUploadStep.setPkgName(npm.getPkgName());
            fileUploadStep.setArtifactPath(npm.getArtifactPath());
            fileUploadStep.setArtifactVersion(version);
        }
        NpmStep npmStep = new NpmStep();
        npmStep.setFileUploadStep(fileUploadStep);

        Set<ContainerVolume> tempDirs = Sets.newHashSet();
        if (npm.isAutoGenerateConfig()) {
            npmStep.setAutoGenerateConfig(npm.isAutoGenerateConfig());
            npmStep.setPublicRepoNames(npm.getPublicRepoNames());
            npmStep.setPrivateRepoNames(npm.getPrivateRepoNames());
            npmStep.setUserHomePath(npm.getUserHomePath());

            String destination = npm.getUserHomePath();
        ...
        CommonJob<NpmStep> commonJob = new CommonJob<NpmStep>()
                .setCloneMode(npm.getCloneMode())
                .setJobType(jobRecord.getJobType())
                .setCommand(npm.getCommand())
    ...
    public void jobSuccessCallback(PipelineRecord pipelineRecord, JobRecord jobRecord, String message, JSONObject data) {
        ...
        if (npm.isUploadArtifact()) {
            ...
            artifactInfoService.saveArtifactInfo(pipelineRecord, ArtifactType.RAW, npm.getPkgRepo(), npm.getPkgName(), version);
        }
